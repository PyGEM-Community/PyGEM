...
def plot_reg_priors(main_glac_rgi, priors_df, reg, rgi_regionsO2, fig_fp):
    ...
    for nreg, regO2 in enumerate(rgi_regionsO2):
        ...
        # Plot histogram
        counts, bins, patches = ax[nrow, ncol].hist(
            kp_values,
            facecolor='grey',
            edgecolor='grey',
            linewidth=0.1,
    ...
    fig.savefig(
        fig_fp + 'priors_kp_O2Regions-' + str(reg) + '.png',
        bbox_inches='tight',
        dpi=300,
    )
    ...
    fig.set_size_inches(6, 6)
    fig.savefig(
        fig_fp + 'priors_tbias_O2Regions-' + str(reg) + '.png',
        bbox_inches='tight',
        dpi=300,
    )
    ...
    if reg not in [19]:
        ...
        for regO2 in rgi_regionsO2:
            ...
            if debug:
                ...
                print(
                    'tbias (mean/std/med/min/max):',
                    np.round(tbias_mean, 2),
                    np.round(tbias_std, 2),
                    np.round(tbias_med, 2),
                    np.round(tbias_min, 2),
                    np.round(tbias_max, 2),
                )
...
def main():
    ...
    partial_function = partial(
        ...
        priors_reg_outpath=args.priors_reg_outpath,
        debug=args.debug,
        plot=args.plot,
    )
