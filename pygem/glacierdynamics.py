...
class MassRedistributionCurveModel(FlowlineModel):
    ...
    def run_until(self, y1, run_single_year=False):
        ...
        if self.check_for_boundaries:
            if self.fls[-1].thick[-1] > 10:
                raise RuntimeError('Glacier exceeds domain boundaries, at year: {}'.format(self.yr))
        # Check for NaNs
        for fl in self.fls:
            if np.any(~np.isfinite(fl.thick)):
                raise FloatingPointError('NaN in numerical solution.')

    def run_until_and_store(self, y1, run_path=None, diag_path=None, store_monthly_step=None):
        ...
        if int(y1) != y1:
            raise InvalidParamsError('run_until_and_store only accepts integer year dates.')

        if not self.mb_model.hemisphere:
            raise InvalidParamsError('run_until_and_store needs a mass-balance model with an unambiguous hemisphere.')
        # time
        ...
        for s, w, b in zip(sects, widths, bucket):
            ...
            varcoords = OrderedDict(time=('time', yearly_time), year=('time', yearly_time))
            ds['ts_section'] = xr.DataArray(s, dims=('time', 'x'), coords=varcoords)
            ds['ts_width_m'] = xr.DataArray(w, dims=('time', 'x'), coords=varcoords)
            if self.is_tidewater:
                ds['ts_calving_bucket_m3'] = xr.DataArray(b, dims=('time',), coords=varcoords)
            run_ds.append(ds)
        ...
        for fl_id, fl in enumerate(self.fls):
            ...
            # MASS REDISTRIBUTION
                ...
                if self.is_tidewater:
                    ...
                    fa_m3 = self._get_annual_frontalablation(heights, fls=self.fls, fl_id=fl_id, year=year, debug=False)
                    if debug:
    ...
    def _get_annual_frontalablation(self, heights, year=None, fls=None, fl_id=None, calving_k=None, debug=False):
        ...
        if glacier_area_t0.sum() > 0:
            try:
                last_above_wl = np.nonzero((fl.surface_h > self.water_level) & (fl.thick > 0))[0][-1]
            except:
                last_above_wl = np.nonzero((fl.bed_h <= self.water_level) & (fl.thick > 0))[0][-1]
        ...
        while (icethickness_change > pygem_prms['sim']['icethickness_advancethreshold']).any() == True:
            ...
            if self.fls[0].surface_h[glac_idx_t0_term] < self.water_level:
                ...
                if self.fls[0].surface_h[glac_idx_t0_term] > elev_term:
                    ...
                    advance_volume = (glacier_area_t0_raw[glac_idx_t0_term] * thick_t0_raw[glac_idx_t0_term]).sum() - (
                        glacier_area_t1[glac_idx_t0_term] * self.fls[0].thick[glac_idx_t0_term]
                    ).sum()
            ...
            if advance_volume > 0:
                glac_idx_bin2add = np.where(
                    self.fls[0].surface_h == self.fls[0].surface_h[np.where(self.fls[0].surface_h < min_elev)[0]].max()
                )[0][0]
                ...
                try:
                    ...
                    glac_idx_terminus_initial = glac_idx_initial[
                        ...
                        / (heights[glac_idx_initial].max() - heights[glac_idx_initial].min())
                        * 100
                        < pygem_prms['sim']['terminus_percentage']
                    ]
    ...
    def _massredistributioncurveHuss(
        ...
        if glac_idx_t0.shape[0] > 3:
            ...
            icethicknesschange_norm[glacier_area_t0 > 0] = (
                (elevrange_norm[glacier_area_t0 > 0] + a) ** gamma + b * (elevrange_norm[glacier_area_t0 > 0] + a) + c
            )
        ...
        bin_volumechange_remaining = bin_volumechange - (
            self.fls[0].section * self.fls[0].dx_meter - section_t0 * self.fls[0].dx_meter
        )
