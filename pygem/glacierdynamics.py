...
class MassRedistributionCurveModel(FlowlineModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        check_for_boundaries: bool, default: True
            ...
            boundaries
        """
        super(MassRedistributionCurveModel, self).__init__(
            flowlines,
            mb_model=mb_model,
            y0=y0,
            inplace=inplace,
    ...
    def updategeometry(self, year, debug=False):
        ...
        for fl_id, fl in enumerate(self.fls):
            ...
            if (
                (self.option_areaconstant)
                or (year < self.spinupyears)
                or (year < self.constantarea_years)
            ):
                # run mass balance
            ...
            # MASS REDISTRIBUTION
                ...
                if self.is_tidewater:
                    ...
                    fa_m3 = self._get_annual_frontalablation(
                        heights, fls=self.fls, fl_id=fl_id, year=year, debug=False
                    )
                    if debug:
                    ...
                    glac_idx_bsl = np.where(
                        (thick_t0 > 0) & (fl.bed_h < self.water_level)
                    )[0]
                    while fa_m3 > 0 and len(glac_idx_bsl) > 0:
                        ...
                        if fa_m3 > vol_last:
                            ...
                            self.mb_model.glac_bin_frontalablation[
                        ...
                        # Otherwise, remove ice from the section
                            ...
                            section_t0[last_idx] = (
                                section_t0[last_idx] - fa_m3 / fl.dx_meter
                            )
                            self.fls[fl_id].section = section_t0
                ...
                if len(section_t0.nonzero()[0]) > 0:
                    ...
                    self._massredistributionHuss(
                        section_t0,
                        thick_t0,
                        width_t0,
                        glac_bin_massbalclim_annual,
                        self.glac_idx_initial[fl_id],
                        heights,
            ...
            year = int(
                year
            )  # required to ensure proper indexing with run_until_and_store (10/21/2020)
    ...
    # %% ----- FRONTAL ABLATION -----
    def _get_annual_frontalablation(
        self, heights, year=None, fls=None, fl_id=None, calving_k=None, debug=False
    ):
        ...
        if glacier_area_t0.sum() > 0:
            try:
                last_above_wl = np.nonzero(
                    (fl.surface_h > self.water_level) & (fl.thick > 0)
                )[0][-1]
            except:
                last_above_wl = np.nonzero(
                    (fl.bed_h <= self.water_level) & (fl.thick > 0)
                )[0][-1]

            if last_above_wl is not None:
    ...
    def _massredistributionHuss(
        ...
        while glacier_volumechange_remaining < 0:
            ...
            glacier_volumechange_remaining_retreated = (
                glacier_volumechange_remaining.copy()
            )
        ...
        while (
            ...
            self.fls[0].thick[glac_idx_advance] = self.fls[0].thick[
                glac_idx_advance
            ] - (
            ...
            if self.fls[0].surface_h[glac_idx_t0_term] < self.water_level:
                ...
                section_updated[glac_idx_t0_term] = (
                    section_updated[glac_idx_t0_term] + advance_section
                )
                ...
                if self.fls[0].surface_h[glac_idx_t0_term] > elev_term:
                    ...
                    thick_reduction = (
                        self.fls[0].surface_h[glac_idx_t0_term] - elev_term
                    )
                    ...
                    self.fls[0].thick[glac_idx_t0_term] = (
                        self.fls[0].thick[glac_idx_t0_term] - thick_reduction
                    )
                    glacier_area_t1 = self.fls[0].widths_m.copy() * self.fls[0].dx_meter
                    ...
                    advance_volume = (
                        ...
                        * thick_t0_raw[glac_idx_t0_term]
                        ...
                        glacier_area_t1[glac_idx_t0_term]
                        * self.fls[0].thick[glac_idx_t0_term]
                    ).sum()
            ...
            if advance_volume > 0:
                glac_idx_bin2add = np.where(
                    self.fls[0].surface_h
                    == self.fls[0]
                    .surface_h[np.where(self.fls[0].surface_h < min_elev)[0]]
                    .max()
                )[0][0]
                ...
                try:
                    minelev_idx = np.where(heights == heights[glac_idx_terminus].min())[
                        0
                    ][0]
                    glac_idx_terminus_removemin = list(glac_idx_terminus)
                    glac_idx_terminus_removemin.remove(minelev_idx)
                    terminus_thickness_avg = np.mean(
                        self.fls[0].thick[glac_idx_terminus_removemin]
                    )
                except:
                    glac_idx_terminus_initial = glac_idx_initial[
                        (heights[glac_idx_initial] - heights[glac_idx_initial].min())
                        / (
                            heights[glac_idx_initial].max()
                    ...
                    minelev_idx = np.where(
                        heights == heights[glac_idx_terminus_initial].min()
                    )[0][0]
                ...
                if self.fls[0].thick[glac_idx_bin2add] > terminus_thickness_avg:
                    ...
                    volume_added2bin = (
                        self.fls[0].section[glac_idx_bin2add] * self.fls[0].dx_meter
                    )
            ...
            if advance_volume > 0:
                ...
                # otherwise, redistribute mass
                    ...
                    glac_bin_massbalclim_annual[glac_idx_t0] = (
                        glacier_volumechange_remaining
                        / glacier_area_t0.sum()
                        / sec_in_year
                    )
                    icethickness_change, glacier_volumechange_remaining = (
                        self._massredistributioncurveHuss(
                            self.fls[0].section.copy(),
                            self.fls[0].thick.copy(),
                            self.fls[0].widths_m.copy(),
                            glac_idx_t0,
                            advance_volume,
                            glac_bin_massbalclim_annual,
                            heights,
    ...
    def _massredistributioncurveHuss(
        self,
        section_t0,
        thick_t0,
        width_t0,
        ...
        if glac_idx_t0.shape[0] > 3:
            ...
            elevrange_norm[glacier_area_t0 > 0] = (
                heights[glac_idx_t0].max() - heights[glac_idx_t0]
            ) / (heights[glac_idx_t0].max() - heights[glac_idx_t0].min())
            ...
            icethicknesschange_norm[glacier_area_t0 > 0] = (
                ...
                + b * (elevrange_norm[glacier_area_t0 > 0] + a)
                + c
            )
        ...
        bin_volumechange_remaining = bin_volumechange - (
            self.fls[0].section * self.fls[0].dx_meter
